...
<template>
  <div class="article_container">
    ...
    <main v-if="tagList?.tags_connection" class="cloud">
      <ul>
        <li v-for=" {color, slug, name, posts_connection: {nodes: {length}, }, } in tagList.tags_connection.nodes.filter((f): f is typeof f & {posts_connection:{nodes:Post[]}} => !!f.posts_connection?.nodes.length)" :key="`post-list?tag=${slug}`" :title="`Posts: ${length}`">
