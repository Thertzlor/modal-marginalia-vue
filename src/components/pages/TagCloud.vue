...
<template>
  <div class="article_container">
    ...
    <main v-if="tagList?.tags" class="cloud">
      <ul>
        <li v-for="{attributes: {color, slug, name, posts: {data: {length}, }, }} in tagList.tags.data.filter((f): f is Present<typeof f, 'attributes'> & {attributes:{posts:{data:PostEntity[]}}} => !!f.attributes?.posts?.data.length)" :title="`Posts: ${length}`" :key="`post-list?tag=${slug}`">
