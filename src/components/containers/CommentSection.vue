...
import type {CommentEntity, Pagination as PaginationType, PaginationArg} from '@/graphql/api';
import {ref} from "vue";
import VueHcaptcha from "@hcaptcha/vue3-hcaptcha";
import {useCommentatorMutation} from '@/graphql/api'
import Pagination from "../navigation/Pagination.vue";

const props = defineProps<{post_id: number, comment_data: CommentEntity[], page?:number, pagination?:PaginationType}>();
...
<template>
  <div class="commentainer">
    <h2>Comments</h2>
    <hr />
    <Pagination term="p" @pg="propagate" v-if="pagination" :page_data="pagination" />
    ...
    <form class="com_forms" id="comment_form" @submit.stop.prevent="formSubmit">
      ...
      <VueHcaptcha @pg="propagate" ref="invisibleHcaptcha" :sitekey="sitekey" theme="dark" size="invisible" @verify="commentSubmit" @expired="onExpire" challengeContainer="challenger" @challenge-expired="onChallengeExpire" @error="onError" />
    </form>
  </div>
