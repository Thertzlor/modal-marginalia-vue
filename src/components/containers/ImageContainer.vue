...
const props = defineProps<{ className:string, customSize?:string, image?:string, caption?:string, alt?:string}>();
...
<template>
  <figure v-if="data && iData" :class="className" class="invisible">
    <a :href="image">
      <img
        :srcset="getSrcSet(iData)" :src="data.url" :alt="(alt||data.alternativeText)??undefined"
