...
defineProps<{list:(Partial<Tag>|SomeOf<Partial<Category>>|undefined)[], taxType:string}>();
</script>
...
<template>
  <template v-if="list.length">
    <RouterLink
      ...
      :custom="false"
      :to="`/post-list?${taxType}=${slug}`" :title="description??undefined" :style="({['--glow_color']: color??undefined})">{{ name }}</RouterLink>
  </template>
